    ...
    public static class PublicClass
    {
        ...
        public class B2BReqJSConverter : JavaScriptConverter
        {
            public override object Deserialize(IDictionary<string, object> dictionary, Type type, JavaScriptSerializer serializer)
            {
                ...
                object value = null;
                if (dictionary.TryGetValue("WAREID", out value))
                    node.WAREID = (string)value;
                if (dictionary.TryGetValue("ROWNO", out value))
                    node.ROWNO = (string)value;
                if (dictionary.TryGetValue("APPLYQTY", out value))
                    node.APPLYQTY = (string)value;
                if (dictionary.TryGetValue("PURPRICE", out value))
                    node.PURPRICE = (string)value;
                if (dictionary.TryGetValue("BATID", out value))
                    node.BATID = (string)value;
                if (dictionary.TryGetValue("MAKENO", out value))
                    node.MAKENO = (string)value;
                if (dictionary.TryGetValue("INVALIDATE", out value))
                    node.INVALIDATE = (string)value;
                if (dictionary.TryGetValue("NOTES", out value))
                    node.NOTES = (string)value;
                return node;
            }
            public override IDictionary<string, object> Serialize(object obj, JavaScriptSerializer serializer)
            {
                ...
                if (node == null)
                    return null;
                if (!string.IsNullOrEmpty(node.WAREID))
                    dic.Add("WAREID", node.WAREID);
                if (!string.IsNullOrEmpty(node.ROWNO))
                    dic.Add("ROWNO", node.ROWNO);
                if (!string.IsNullOrEmpty(node.APPLYQTY))
                    dic.Add("APPLYQTY", node.APPLYQTY);
                if (!string.IsNullOrEmpty(node.PURPRICE))
                    dic.Add("PURPRICE", node.PURPRICE);
                if (!string.IsNullOrEmpty(node.BATID))
                    dic.Add("BATID", node.BATID);
                if (!string.IsNullOrEmpty(node.MAKENO))
                    dic.Add("MAKENO", node.MAKENO);
                if (!string.IsNullOrEmpty(node.INVALIDATE))
                    dic.Add("INVALIDATE", node.INVALIDATE);
                if (!string.IsNullOrEmpty(node.NOTES))
                    dic.Add("NOTES", node.NOTES);
                return dic;
            }
